...
//                    King *f(color,i,j);
    ...
    ChessField::updateField();
    for(int i = 0; i<8;i++)
    {
        for(int j=0;j<8;j++)
        {
            if(ChessField::numOfBlackBeaters(i,j)!=0||ChessField::numOfWhiteBeaters(i,j)!=0)
            {
                if(ChessField::numOfBlackBeaters(i,j)>=ChessField::numOfWhiteBeaters(i,j))
                {
                    arr[i][j]=new Score(this,ChessField::numOfBlackBeaters(i,j)-ChessField::numOfWhiteBeaters(i,j),false,i,j);
                }
                else
                {
                    arr[i][j]=new Score(this,-ChessField::numOfBlackBeaters(i,j)+ChessField::numOfWhiteBeaters(i,j),true,i,j);
                }
            }
        }
    }
